      ...
      <div className="flex items-center gap-1.5 md:gap-3 flex-shrink-0">
        ...
        <div className="relative hidden lg:flex items-center h-12 ml-2" ref={dropdownRef}>
          ...
          >
            ...
            <span className="text-sm font-black text-slate-900 truncate max-w-[100px]">{userName}</span>
          ...
          {profileDropdownOpen && (
            <div className="absolute right-0 top-full mt-2 w-52 bg-white/95 backdrop-blur-xl border border-white/40 rounded-2xl shadow-2xl py-2 z-[1001] animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              <button onClick={() => navigateTo('/suporte')} className="w-full flex items-center gap-3 px-4 py-2 text-sm font-bold text-slate-700 hover:bg-[#006c55]/5 hover:text-[#006c55]">
                <LifeBuoy size={16} /> Suporte
              </button>
              <div className="h-px bg-slate-100 my-2 mx-4"></div>
              <a href="#" onClick={handleLogout} className="flex items-center gap-3 px-4 py-2 text-sm font-black text-red-500 hover:bg-red-50">
                <LogOut size={16} /> Sair
              </a>
            </div>
          )}
